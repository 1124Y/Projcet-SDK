...
namespace ai_chat_sdk
{
    ...
    void ChatSDK::registerAllProvider(const std::vector<std::shared_ptr<Config>> &configs)
    {
        ...
        // Ollama模型提供者注册
        if (!_llmManager.isModelAvailable("ollama-chat"))
        {
            auto ollamaProvider = std::make_unique<OllamaLLMProvider>();
            _llmManager.registerProvider("ollama-chat", std::move(ollamaProvider));
            INFO("ollama-chat provider registered successed");
        }
    }
    // 初始化所支持的模型
