...
namespace ai_chat_sdk
{
                                                     ...
                                                     std::function<void(const std::string &, bool)> callback)
    {
        ...
        // 流式处理变量
        std::string buffer;        // 保存增量结果
        bool gotError = false;     // 是否已经收到错误响应
        std::string errorMsg;      // 错误信息
        int statusCode = 0;        // http响应状态码
        bool streamFinish = false; // 流式响应是否已经结束
        std::string fullData;      // 保存完整结果（全量结果）
        ...
        request.content_receiver = [&](const char *data, size_t dataLen, size_t offset, size_t totalLength)
        {
            ...
            while ((pos = buffer.find("\n", pos)) != std::string::npos)
            {
                ...
                // 反序列化
                Json::Value chunkJson;                 // json对象
                Json::CharReaderBuilder readerBuilder; // json解析器
                std::string errors;                    // json解析错误信息
                std::istringstream chunkStream(chunk); // 将字符串转换为输入流
